...
const PAPER = "#f7f4ec";
const INITIAL = { spacing: 8, weight: 1.15, taper: 2.2, outline: 1.6, contrast: 68, angle: -12, light: 35, lineStyle: "straight", wave: 8, dottedEnds: true, dottedFade: 58 };
const PRESETS = [
...
function hatchMaterial() {
  ...
  material.fragmentShader = material.fragmentShader.replace("uniform float uSpacing,uWeight,uContrast,uAngle,uLineStyle,uWave,uDottedEnds,uDottedFade;", "uniform float uSpacing,uWeight,uTaper,uContrast,uAngle,uLineStyle,uWave,uDottedEnds,uDottedFade;");
...
function dottedRun(points, strokeWidth, settings, ink, allowDotted = true) {
  const d = (list) => list.map((point, index) => (index ? "L" : "M") + point.x.toFixed(1) + " " + point.y.toFixed(1)).join(""), strength = (settings.dottedFade ?? 58) / 100;
  ...
  if (!allowDotted || !settings.dottedEnds || strength < .01 || points.length < 5) return '<path stroke-width="' + strokeWidth.toFixed(2) + '" d="' + d(points) + '"/>';
  ...
  return '<path stroke-width="' + strokeWidth.toFixed(2) + '" d="' + d(solid) + '"/>' + segments.join("");
}
...
function applyViewportAppearance(runtime, paper, ink, settings) {
  ...
  runtime.scene.traverse((item) => {
    ...
    if (!uniforms?.uWeight || !uniforms?.uTaper) return;
    uniforms.uInk.value.set(ink); uniforms.uPaper.value.set(paper); uniforms.uSpacing.value = settings.spacing;
    uniforms.uWeight.value = settings.weight; uniforms.uTaper.value = settings.taper ?? 2.2; uniforms.uContrast.value = settings.contrast / 100;
