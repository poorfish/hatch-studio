...
export function App() {
  ...
  const switchMode = (nextMode) => {
    ...
    if (nextMode === "image" && model?.kind !== "image") { notify("Upload an image to use Image mode"); return; }
    if (nextMode === "model" && model?.kind === "image") { notify("Choose a 3D preset or model"); return; }
    setMode(nextMode);
  };
  ...
  return <main className={"app-shell" + (showPanel ? "" : " panel-hidden") + (isFullscreen ? " is-fullscreen" : "")} id="top">
    ...
    <aside className={"control-panel" + (showPanel ? "" : " hidden") + (isDraggingPanel ? " panel-dragging" : "")} style={panelStyle}>
      <div className="panel-brand" onPointerDown={startPanelDrag} title={isFullscreen ? "Drag to move settings" : undefined}><span>HatchKit</span><Sparkles size={15}/></div><div className="panel-divider"/><div className="mode-switch"><span>Mode</span><div className="segmented"><button className={mode === "model" ? "selected" : ""} onClick={() => switchMode("model")}>3D model</button><button className={mode === "image" ? "selected" : ""} onClick={() => switchMode("image")}>Image</button></div></div>
